...
// Security: Hide server technology stack header
app.disable('x-powered-by');

// Security Middleware: Set strict HTTP Security Headers
app.use((req, res, next) => {
  res.setHeader('X-Frame-Options', 'SAMEORIGIN');
  res.setHeader('X-Content-Type-Options', 'nosniff');
  res.setHeader('X-XSS-Protection', '1; mode=block');
  res.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin');
  res.setHeader('Permissions-Policy', 'nfc=(self), camera=(), microphone=(), geolocation=()');
...
app.use('/valeo', express.static(path.join(__dirname, 'public')));
app.use('/mifare', express.static(path.join(__dirname, 'public')));
app.use('/nfc-okuyucu', express.static(path.join(__dirname, 'public')));
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`ğŸš€ GÃ¼venli Mifare NFC Kart Okuyucu sunucusu http://0.0.0.0:${PORT} portunda baÅŸarÄ±yla Ã§alÄ±ÅŸÄ±yor!`);
});
...

{
  "name": "nfc-okuyucu",
  "version": "1.0.0",
  "description": "Mifare & Web NFC Mobil Kart Okuyucu Terminali",
  "main": "server.js",
  "scripts": {
...
========== INDEX.HTML (GÃ¶rsel ArayÃ¼z ve Ana Kodlar) ==========
...
<head>
  ...
  <title>Valeo SmartScan Terminal | Valeo Automotive Systems</title>
...
<body>
  <div id="root">
    <div style="min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: #0B131D; color: #82E600; font-family: sans-serif; text-align: center;">
      <h2 style="font-size: 24px; font-weight: 900; margin-bottom: 8px;">VALEO SMARTSCAN TERMINAL</h2>
      <p style="font-size: 13px; color: #94A3B8;">Mifare & Web NFC Kart Okuyucu Terminali YÃ¼kleniyor...</p>
    </div>
  </div>
...
// DÃ¼nya StandartlarÄ±nda Orijinal Fiziki Kart Okuyucu (HID / Mifare 2.4kHz Piezo Beep) UyarÄ± Sesi
const playOriginalCardReaderBeep = () => {
  try {
    ...
    // 2400 Hz: DÃ¼nya genelindeki tÃ¼m fiziki kart okuyucu (HID / Suprema / Mifare) standart piezo bip tonu
    osc.type = 'sine';
    ...
    console.log("Original card reader beep error:", e);
  }
};
...
function App() {
  ...
  const startNfcScan = async () => {
    try {
      ...
      ndef.addEventListener("readingerror", event => {
        setReadError("Kart okuma hatasÄ± oluÅŸtu. LÃ¼tfen kartÄ± tekrar dokundurun.");
        try {
      ...
      ndef.addEventListener("reading", event => {
        ...
        let realUid = "UID-OKUNAMADI";
        if (serialNumber) {
          realUid = serialNumber.toUpperCase();
        } else if (event.id) {
          realUid = event.id.toUpperCase();
        }
        try {
          triggerVibration();
          playOriginalCardReaderBeep();
        } catch (e) {}
        ...
        if (message && message.records && message.records.length > 0) {
          ...
          for (const record of message.records) {
            if (record.recordType === 'text') {
              ...
              decodedMessages.push(`KayÄ±t Tipi: ${record.recordType}`);
            }
          }
          setMessages(decodedMessages);
        } else {
          decodedMessages = ["Fiziksel Ã‡ip UID Okundu."];
          setMessages(decodedMessages);
        }
        const newRecord = {
          id: Date.now(),
      ...
      if (error.name === 'NotAllowedError') {
        ...
        setReadError("NFC Ä°zni EngellenmiÅŸ! LÃ¼tfen Chrome adres Ã§ubuÄŸundaki KÄ°LÄ°T ikonuna basÄ±p NFC Ä°znine Ä°ZÄ°N VER deyin.");
      } else if (error.name === 'NotSupportedError') {
        setReadError("Web NFC bu cihazda desteklenmiyor.");
        setHasNfcSupport(false);
      } else {
        setReadError(`NFC BaÅŸlatÄ±lamadÄ±: ${error.message || 'Bilinmeyen Hata'}`);
      }
    }
  };
  const handleManualAdd = e => {
    ...
    const uidStr = manualUid.trim().toUpperCase();
    try {
      triggerVibration();
      playOriginalCardReaderBeep();
    } catch (e) {}
    setCardUid(uidStr);
    setLastReadTime(new Date().toLocaleTimeString('tr-TR'));
    const msg = ["Manuel Girilen Ã‡ip UID KaydÄ±"];
    setMessages(msg);
    setReadError(null);
    const newRecord = {
      id: Date.now(),
  ...
  const downloadHistory = () => {
    ...
    const headers = "Tarih & Saat,Kart UID,Ä°Ã§erik Verisi\n";
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `min-h-screen transition-colors duration-500 ${flashSuccess ? 'bg-emerald-100' : 'bg-gray-50'} flex flex-col font-sans max-w-md mx-auto w-full shadow-xl relative pb-20 border-x border-gray-200`
  }, /*#__PURE__*/React.createElement("header", {
    className: "bg-white border-b border-emerald-100 p-4 sticky top-0 z-30 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center space-x-2.5"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bg-emerald-500 text-white p-2 rounded-xl shadow-md shadow-emerald-500/20"
  ...
  }, "SmartScan Terminal"), /*#__PURE__*/React.createElement("p", {
    className: "text-[11px] text-emerald-600 font-semibold tracking-wide"
  }, "Mifare & NFC Kart Okuyucu"))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center space-x-1.5 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full text-xs font-medium"
  }, /*#__PURE__*/React.createElement("span", {
    className: `w-2.5 h-2.5 rounded-full ${isScanning ? 'bg-emerald-500 animate-ping' : 'bg-gray-400'}`
  }), /*#__PURE__*/React.createElement("span", {
    className: `text-[10px] font-bold tracking-wider ${isScanning ? 'text-emerald-700' : 'text-gray-500'}`
  }, isScanning ? 'TARAMA AKTÄ°F' : 'HAZIR')))), /*#__PURE__*/React.createElement("main", {
    className: "flex-grow p-4 flex flex-col space-y-4 overflow-y-auto"
  }, isIOSDevice && /*#__PURE__*/React.createElement("div", {
    className: "bg-amber-50 border border-amber-200 p-3.5 rounded-2xl text-amber-800 text-xs space-y-1 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-amber-900 flex items-center text-sm"
  }, "\uF8FF iOS K\u0131s\u0131tlamas\u0131 Uyar\u0131s\u0131"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs leading-relaxed"
  }, "Apple iOS Web NFC'yi engeller. Test i\xE7in a\u015Fa\u011F\u0131daki ", /*#__PURE__*/React.createElement("strong", null, "\"Manuel UID Gir\""), " butonunu kullanabilirsiniz.")), hasNfcSupport === false && !isIOSDevice && /*#__PURE__*/React.createElement("div", {
    className: "bg-amber-50 border border-amber-200 p-4 rounded-2xl text-amber-800 text-xs space-y-2 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-amber-900 text-sm flex items-center"
  }, "\u26A0\uFE0F Web NFC Donan\u0131m Uyar\u0131s\u0131"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-amber-700 leading-relaxed"
  }, "Web NFC teknolojisi ", /*#__PURE__*/React.createElement("strong", null, "Android Chrome + HTTPS"), " gerektirir."), /*#__PURE__*/React.createElement("div", {
    className: "bg-white p-2.5 rounded-xl border border-amber-300 font-mono text-xs text-emerald-700 text-center break-all"
  }, window.location.origin + window.location.pathname)), permissionStatus === 'denied' && /*#__PURE__*/React.createElement("div", {
    className: "bg-red-50 border-2 border-red-500 text-red-900 p-4 rounded-2xl text-xs space-y-2 animate-pulse shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-red-700 text-sm"
  }, "\uD83D\uDEAB Chrome NFC \u0130zni Engellenmi\u015F"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-red-800"
  }, "Adres \xE7ubu\u011Fundaki \uD83D\uDD12 ", /*#__PURE__*/React.createElement("strong", null, "Kilit \u0130konuna"), " bas\u0131p ", /*#__PURE__*/React.createElement("strong", null, "NFC \u2794 \u0130zin Ver"), " yap\u0131n\u0131z.")), activeTab === 'scan' && /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col flex-grow space-y-4 fade-in"
  }, readError && /*#__PURE__*/React.createElement("div", {
    className: "bg-amber-50 border border-amber-300 text-amber-900 p-3.5 rounded-2xl text-xs text-center font-medium shadow-sm animate-pulse"
  }, readError), /*#__PURE__*/React.createElement("div", {
    className: `bg-white rounded-3xl p-6 text-center transition-all duration-300 shadow-xl border ${flashSuccess ? 'border-emerald-500 ring-4 ring-emerald-100' : 'border-gray-100'} my-auto`
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex justify-center my-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: `p-6 rounded-full transition-all duration-500 border ${isScanning ? 'bg-emerald-50 text-emerald-600 green-pulse border-emerald-200' : 'bg-gray-100 text-gray-400 border-gray-200'}`
  }, /*#__PURE__*/React.createElement("svg", {
    className: "w-16 h-16",
    fill: "none",
    stroke: "currentColor",
  ...
  }, "Fiziksel Kart ID (UID)"), lastReadTime && /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] font-mono font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full"
  }, "Son Okuma: ", lastReadTime)), /*#__PURE__*/React.createElement("div", {
    className: "bg-gray-900 text-white border border-gray-800 rounded-2xl py-5 px-3 shadow-inner"
  }, /*#__PURE__*/React.createElement("span", {
    className: `font-mono text-2xl font-extrabold tracking-widest break-all ${cardUid ? 'text-emerald-400' : 'text-gray-500'}`
  }, cardUid ? cardUid : isScanning ? 'KARTI DOKUNDURUN...' : 'BEKLEMEDE')), messages.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "mt-4 pt-4 border-t border-gray-100 text-left"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 text-center"
  }, "\xC7ip \u0130\xE7erik Verisi"), /*#__PURE__*/React.createElement("div", {
    className: "bg-emerald-50 p-3 rounded-xl border border-emerald-100 font-mono text-xs text-emerald-900"
    ...
    className: "bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs py-3.5 px-4 rounded-2xl font-bold flex items-center justify-center space-x-2 shadow-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-2.5 h-2.5 rounded-full bg-emerald-600 animate-ping"
  }), /*#__PURE__*/React.createElement("span", null, "TARAMA AKT\u0130F (Kart\u0131 Arka Kameraya Yakla\u015Ft\u0131r\u0131n)")), /*#__PURE__*/React.createElement("button", {
    onClick: cancelNfcScan,
    className: "w-full bg-red-500 hover:bg-red-600 text-white font-bold py-4 rounded-2xl shadow-lg transition text-sm tracking-wide uppercase"
  }, "Taramay\u0131 Durdur")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    onClick: startNfcScan,
    disabled: hasNfcSupport === false,
    className: `w-full font-black py-4 rounded-2xl shadow-lg shadow-emerald-500/20 transition-all duration-200 text-base tracking-wider uppercase ${hasNfcSupport === false ? 'bg-gray-200 text-gray-400 cursor-not-allowed border border-gray-300' : 'bg-emerald-500 hover:bg-emerald-600 text-white'}`
  }, "S\xFCrekli Taramay\u0131 Ba\u015Flat"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowManualModal(true),
    className: "w-full bg-white hover:bg-gray-50 text-gray-800 font-bold py-3.5 rounded-2xl border border-gray-200 shadow-sm transition text-xs flex items-center justify-center space-x-2"
  }, /*#__PURE__*/React.createElement("svg", {
  ...
  })), /*#__PURE__*/React.createElement("span", null, "Manuel UID Gir")))), /*#__PURE__*/React.createElement("div", {
    className: "pt-1"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowTroubleshoot(!showTroubleshoot),
    className: "w-full bg-white text-gray-600 hover:text-gray-900 border border-gray-200 rounded-2xl py-3 px-4 text-xs font-semibold flex justify-between items-center transition shadow-sm"
  }, /*#__PURE__*/React.createElement("span", null, "\uD83D\uDCA1 Kart okuma ipu\xE7lar\u0131"), /*#__PURE__*/React.createElement("span", {
    className: "text-emerald-600 font-bold"
  }, showTroubleshoot ? 'â–²' : 'â–¼')), showTroubleshoot && /*#__PURE__*/React.createElement("div", {
    className: "bg-white border border-gray-200 rounded-2xl p-4 mt-2 text-xs text-gray-700 space-y-2 fade-in shadow-sm"
  }, /*#__PURE__*/React.createElement("div", null, "\u2022 ", /*#__PURE__*/React.createElement("strong", null, "K\u0131l\u0131f:"), " Kal\u0131n plastik veya m\u0131knat\u0131sl\u0131 k\u0131l\u0131flar\u0131 \xE7\u0131kar\u0131n\u0131z."), /*#__PURE__*/React.createElement("div", null, "\u2022 ", /*#__PURE__*/React.createElement("strong", null, "Konum:"), " Kart\u0131 telefonun arkas\u0131na/kameras\u0131na 1-2 saniye sabitleyin."), /*#__PURE__*/React.createElement("div", null, "\u2022 ", /*#__PURE__*/React.createElement("strong", null, "Frekans:"), " 13.56 MHz Mifare \xE7ipleri desteklenir.")))), activeTab === 'history' && /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col flex-grow space-y-3 fade-in"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex justify-between items-center px-1"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    className: "text-base font-bold text-gray-900"
  }, "Okuma Ge\xE7mi\u015Fi"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-gray-500"
  }, "Toplam Kay\u0131t: ", scanHistory.length)), /*#__PURE__*/React.createElement("button", {
    onClick: downloadHistory,
    disabled: scanHistory.length === 0,
    className: `text-xs font-bold py-2.5 px-4 rounded-xl shadow transition flex items-center space-x-1.5 ${scanHistory.length === 0 ? 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600 text-white'}`
  }, /*#__PURE__*/React.createElement("svg", {
    className: "w-4 h-4",
    fill: "none",
    stroke: "currentColor",
  ...
  })), /*#__PURE__*/React.createElement("span", null, "CSV \u0130ndir"))), scanHistory.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "flex-grow flex flex-col items-center justify-center text-gray-400 py-12"
  }, /*#__PURE__*/React.createElement("svg", {
    className: "w-16 h-16 mb-3 text-gray-200",
    fill: "none",
    stroke: "currentColor",
    viewBox: "0 0 24 24"
  }, /*#__PURE__*/React.createElement("path", {
    ...
    className: "text-xs"
  }, "Hen\xFCz kay\u0131t yok.")) : /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden"
  }, /*#__PURE__*/React.createElement("ul", {
    className: "divide-y divide-gray-100 max-h-[60vh] overflow-y-auto"
  }, scanHistory.map(rec => /*#__PURE__*/React.createElement("li", {
    key: rec.id,
    className: "p-3.5 hover:bg-emerald-50/50 transition"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex justify-between items-start"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-mono font-bold text-emerald-700 text-sm"
  }, rec.uid), /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] font-mono text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full"
  }, rec.timestamp)), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-gray-600 mt-1"
  }, rec.data))))))), showManualModal && /*#__PURE__*/React.createElement("div", {
    className: "fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-3xl p-6 w-full max-w-xs shadow-2xl fade-in space-y-4 text-gray-900 border border-gray-100"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold text-gray-900 text-base flex items-center"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-2.5 h-2.5 rounded-full bg-emerald-500 mr-2"
  }), "Manuel Kart UID Gir"), /*#__PURE__*/React.createElement("form", {
    onSubmit: handleManualAdd,
    className: "space-y-4"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    className: "block text-xs font-semibold text-gray-500 mb-1"
  }, "Kart UID:"), /*#__PURE__*/React.createElement("input", {
    type: "text",
    placeholder: "\xD6rn: 04:A2:8B:F2:19",
    value: manualUid,
    onChange: e => setManualUid(e.target.value),
    className: "w-full bg-gray-50 border border-gray-300 text-gray-900 rounded-xl p-3 font-mono text-sm uppercase focus:ring-2 focus:ring-emerald-500 focus:outline-none",
    autoFocus: true
  })), /*#__PURE__*/React.createElement("div", {
    ...
    type: "submit",
    className: "flex-1 bg-emerald-500 text-white font-extrabold py-2.5 rounded-xl text-xs shadow-md shadow-emerald-500/20"
  }, "Kaydet"))))), /*#__PURE__*/React.createElement("nav", {
    className: "fixed bottom-0 w-full max-w-md bg-white border-t border-gray-200 flex z-30 shadow-lg"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => handleTabChange('scan'),
    className: `flex-1 flex flex-col items-center justify-center py-3 text-[11px] font-bold transition-all ${activeTab === 'scan' ? 'text-emerald-600' : 'text-gray-400 hover:text-gray-600'}`
  }, /*#__PURE__*/React.createElement("svg", {
    ...
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: activeTab === 'scan' ? 2.2 : 1.5,
    d: "M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"
  })), /*#__PURE__*/React.createElement("span", null, "Taray\u0131c\u0131")), /*#__PURE__*/React.createElement("button", {
    onClick: () => handleTabChange('history'),
    className: `flex-1 flex flex-col items-center justify-center py-3 text-[11px] font-bold transition-all ${activeTab === 'history' ? 'text-emerald-600' : 'text-gray-400 hover:text-gray-600'}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "relative"
  }, /*#__PURE__*/React.createElement("svg", {
    className: "w-5 h-5 mb-1",
    ...
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: activeTab === 'history' ? 2.2 : 1.5,
    d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 022 2h2a2 2 0 022-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"
  })), scanHistory.length > 0 && /*#__PURE__*/React.createElement("span", {
    className: "absolute -top-1 -right-2 bg-emerald-500 text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded-full"
  }, scanHistory.length)), /*#__PURE__*/React.createElement("span", null, "Kay\u0131tlar"))));
}
